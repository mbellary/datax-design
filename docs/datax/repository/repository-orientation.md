...
This repository implements Datax CLI, a local data engineering agent. The root README
states that Datax CLI runs locally on a user's computer and can be used from the
...
- `datax-rs/`: main Rust workspace.
- `datax-cli/`: npm/package wrapper and scripts for CLI distribution.
...
The Rust workspace is declared in `datax-rs/Cargo.toml`. The workspace contains
many crates, and crate package names are prefixed with `datax-`.
...
| Persistence | `chat-store`, `rollout`, `state`, `datax-home`, `message-history`, `keyring-store` |
| Model/backend access | `backend-client`, `model-provider`, `model-provider-info`, `models-manager`, `datax-api` |
| Extensions | `plugin`, `skills`, `core-plugins`, `core-skills`, `ext/*`, `mcp-server`, `datax-mcp` |
...
one big datax agent file. The repository separates:
